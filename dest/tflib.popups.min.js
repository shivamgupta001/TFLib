...
!function() {
    function clone() {
        var template, templates = this.querySelectorAll("template"), fragments = [], i = 0;
        if (support) {
            for (template = this.cloneNode(!0), templates = template.content.querySelectorAll("template"); templates[i]; i++) templates[i].clone = clone;
            return template;
        }
        for (;templates[i]; i++) fragments.push(templates[i].content);
        for (template = this.cloneNode(!0), template.content = this.content, template.clone = clone, 
        templates = template.querySelectorAll("template"), i = 0; templates[i]; i++) templates[i].content = fragments[i], 
        templates[i].clone = clone;
        return template;
    }
    var support = "content" in document.createElement("template");
    if (!support) for (var template, content, fragment, node, j, templates = Array.prototype.slice.call(document.getElementsByTagName("template")), i = 0; template = templates[i++]; ) {
        for (content = template.children, fragment = document.createDocumentFragment(), 
        j = 0; node = content[j]; j++) fragment.appendChild(node);
        template.content = fragment;
    }
    for (var template, templates = document.querySelectorAll("template"), i = 0; template = templates[i++]; ) template.clone = clone;
}(), TFLib.TFConstants = {
    POPUP: {
        ERROR: "Error",
        INFO: "Info",
    ...
    var modalpopup = {
        ...
        _initialize: function() {
            ...
            this.modalOpenCallback = me.modalOnOpen || "", this.dataTemplate = me.dataTemplate || null, 
        ...
        _generateTemplate: function() {
            var el = [ '<div id="' + this.dynamicId + '" class="tf-modal-outer">', '<section draggabe="true" class="tf-modal-inner" tabindex="1">', '<header class="tf-modal-header">', '<span class="tf-modal-title">' + this.title + "</span>", '<a  class= "tf-modal-close-btn"></a>', "</header>", '<section class="tf-modal-body"></section>', '<footer class="tf-modal-footer ' + (this.footerVisible ? "" : "tf-display--none") + '"></footer>', "</section>", "</div>" ].join("\n");
        ...
        _render: function() {
            this.onConfig || (document.body.appendChild(this.outerComp), this.innerComp.focus(), 
        ...
        _handleModalCloseBtnClick: function(e) {
            ...
            "" != this.modalCloseCallback && (toDestroy = this.modalCloseCallback.call(me, e)), 
    ...
    var standardpopup = {
        ...
        _attachProperties: function() {
            ...
            me.okComp = this.okComp, me.handleKeyDown = this.handleKeyDown, this.okComp && (me.popupOnOk = this.popupOnOk, 
            this.okComp.shared = me), this.cancelComp && (me.popupOnCancel = this.popupOnCancel, 
        ...
        _render: function() {
            ...
            this.okComp && this.cancelComp && this.handleCancelBlur.call(me.cancelComp);
        },
        ...
        handleCancelBlur: function(e) {
            ...
            this.shared.okComp.setAttribute("tabindex", 1e4));
        },
