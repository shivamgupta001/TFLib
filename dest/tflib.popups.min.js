...
TFLib.TFConstants = {
    ...
    var modalpopup = {
        ...
        _handlePolyfill: function() {
            !function(d) {
                if ("content" in d.createElement("template")) return !1;
                for (var elPlate, qContent, contentLen, docContent, qPlates = d.getElementsByTagName("template"), plateLen = qPlates.length, x = 0; x < plateLen; ++x) {
                    for (elPlate = qPlates[x], qContent = elPlate.childNodes, contentLen = qContent.length, 
                    docContent = d.createDocumentFragment(); qContent[0]; ) docContent.appendChild(qContent[0]);
